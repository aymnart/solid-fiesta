...
export const updateSecuritySettings = async (data: z.infer<typeof securityFormSchema>) => {
  ...
  if (validatedFields.data.two_factor === false) {
    try {
      ...
      console.warn("Two-factor confirmation record not found or already deleted:", error)
    }
